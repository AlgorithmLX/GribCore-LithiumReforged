package gribland.gribcore.mixin.lithium;
...
public class FabricLithiumMixinPlugin implements IMixinConfigPlugin {
    private static final String MIXIN_PACKAGE_ROOT = "gribland.gribcore.mixin";

    private final Logger logger = LogManager.getLogger("GribCore");
    private LithiumConfig config;

    @Override
    public void onLoad(String mixinPackage) {
        try {
            this.config = LithiumConfig.load(new File("./config/gribcore-mixins.properties"));
        } catch (Exception e) {
            throw new RuntimeException("Could not load configuration file for GribCore", e);
        }

        this.logger.info("Loaded configuration file for GribCore: {} options available, {} override(s) found",
