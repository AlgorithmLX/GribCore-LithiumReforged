package gribland.gribcore.lithium.common.shapes.pairs;
...
public final class LithiumDoublePairList implements IndexMerger {
    ...
    @Override
    public boolean forMergedIndexes(IndexConsumer p_197855_1_) {
        int l = this.pairs.size() - 1;

        for (int i = 0; i < l; i++) {
            if (!p_197855_1_.merge(this.indicesFirst[i], this.indicesSecond[i], i)) {
                return false;
            }
        }
